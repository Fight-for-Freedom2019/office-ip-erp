...
const getters = {
  ...
  renewalOfficialFee: state =>
    state.renewalOfficialFee ? state.renewalOfficialFee : [],
  renewalAgentFee: state =>
    state.renewalAgentFee ? state.renewalAgentFee : [],
  renewalMail: state => (state.renewalMail ? state.renewalMail : [])
};

const mutations = {
  setEstimateDetailData: (state, data) => {
    state.data = data;
  },
  setEstimateLoading(state, boolean) {
...
const actions = {
  refreshEstimateDetail(
    ...
    if (!id) return;
    const url = rootState.status ? URL : URL;
