        ...
        return {
            ...
            tableOption: {
                ...
                header_btn: [
                    ...
                    // {type: 'export'},
                    { type: "control" },
                    {                        type: "dropdown", label: "批量操作", map_if: "/task/btn/save",
                        items: [
                            {
                                text: "下载CPC压缩包",
                                click: () => {
                ...
                columns: [
                    ...
                    { type: "text", label: "客户", prop: "customer", render_simple: "name", render_obj: "project", render_header: true, min_width: "200" },
                    { type: "text", label: "通知书ID", prop: "cpc_id", min_width: "145" },
                    { type: "text", label: "通知书代码", prop: "cpc_code", min_width: "90" },
                    { type: "text", label: "通知书名称", prop: "cpc_file_type", render_simple: "name", min_width: "150", render_header: true, },
                    { type: "text", label: "发文序列号", prop: "cpc_mail_serial", min_width: "160" },
                    {                        type: "text", label: "PDF", prop: "file", min_width: "90", render: (h, item, row) => {
                            if (row.file == null) return;
                            return h("span", {}, [h("a", { attrs: { target: "_blank", href: row.file.viewUrl }, on: { click: e => { e.stopPropagation(); } } }, `查看`), h("a", { attrs: { target: "_blank", href: row.file.downloadUrl }, on: { click: e => { e.stopPropagation(); } }, style: { marginLeft: "5px" } }, `下载`)]);                        }
                    },
                    {                        type: "text", label: "压缩包", prop: "cpc_file", min_width: "60",
                        render: (h, item, row) => {
                    ...
                    {                        type: "text", label: "匹配案件", prop: "project", min_width: "200",
                        render: (h, item) => {
                            return item == null
                                ? ""
                    ...
                    { type: "text", label: "上传用户", prop: "creator_user", render_simple: "name", min_width: "80" },
                    { type: "text", label: "上传时间", prop: "creation_time", render_header: true, min_width: "160" },
                    { type: "text", label: "导入状态", prop: "is_imported", render_simple: "name", render_header: true, min_width: "100" },
                    // { type: "text", label: "导入时间", prop: "import_date", render_header: true, min_width: "110" },
                    // { type: "text", label: "导入用户", prop: "import_user", render_simple: "name", min_width: "110" },
                    ...
                    //     min_width: "100"
                    //   },
                    // { type: "text", label: "官方绝限", prop: "cpc_deadline", render_header: true, min_width: "100" },
