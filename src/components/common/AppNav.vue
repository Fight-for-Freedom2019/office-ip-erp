...
export default {
  ...
  computed: {
    ...mapGetters(["windowWidth", "menusMap"]),
    path() {
    ...
    menuType: {
      ...
      set(val) {
        ...
        const showChildrenMenu = arr[0].menu.filter(d =>
          this.menusMap.get(d.path)
        );
        if (showChildrenMenu[0] && showChildrenMenu[0]["path"]) {
          if (
            showChildrenMenu[0]["children"] &&
            showChildrenMenu[0]["children"][0]["path"]
          ) {
