<template>
  ...
  >
    <div class="app-import">
      ...
      <el-table height="250" style="width: 100%" empty-text="暂无可导入数据" :data="tableData">
        <el-table-column fixed="left" label="序号" width="80">
          <template slot-scope="scope">
            <span style="color: #20a0ff">{{ scope.$index + 1 }}</span>
          </template>
        </el-table-column>
        ...
        <template v-for="(col, index) in columns">
          ...
          <template v-else-if="col.type == 'array'">
            ...
            >
              <template slot-scope="scope">
                <el-tag
                  v-for="(item, i) in arrayRender(scope['row'],col)"
...
export default {
  ...
  computed: {
    ...mapGetters(["importLoading", "menusMap"]),
    config() {
  ...
  methods: {
    ...
    design() {
      ...
      o.project = {
        name: serial.split("-")[1],
        id: this.$refs.project.getSelected()[0].id
      };
